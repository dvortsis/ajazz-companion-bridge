...
It runs quietly in your Windows System Tray and translates the USB signals from your device into network commands that Companion understands.
...
## Supported Devices
This bridge automatically detects and configures itself for the following hardware:
* **Ajazz AKP05** (5x5 grid)
...
### When and How to Change the Port (`COMPANION_PORT`)
* **Default (`16622`)**: This is the official default port for the Companion Satellite API.
* **When to Change**: Only change this if you have another piece of software already using port 16622, forcing you to change the Satellite API port inside the Companion settings. 
* **How to Change**: If Companion is set to listen on `16625`, change the `.env` file to read `COMPANION_PORT=16625`.
...
## Part 6: Windows Auto-Start (Headless Mode)
